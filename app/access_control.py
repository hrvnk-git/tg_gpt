...
class AccessControl:
    ...
    async def seed_if_empty(self) -> None:
        if not self._settings.allow_all_users and await self._redis.scard(self._allowed_key) == 0:
            if self._settings.allowed_user_ids:
                await self._redis.sadd(
                    self._allowed_key,
                    *[str(x) for x in self._settings.allowed_user_ids],
                )

        if not self._settings.allow_all_admins and await self._redis.scard(self._admin_key) == 0:
            if self._settings.admin_user_ids:
                await self._redis.sadd(
                    self._admin_key,
                    *[str(x) for x in self._settings.admin_user_ids],
                )
